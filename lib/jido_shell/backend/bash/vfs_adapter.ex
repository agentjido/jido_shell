if Code.ensure_loaded?(Bash.Filesystem) do
  ...
  defmodule Jido.Shell.Backend.Bash.VfsAdapter do
    @moduledoc """
    Placeholder for the optional `:bash` filesystem adapter.

    The real adapter is compiled only when `:bash` is available. Without it,
    `Jido.Shell.Backend.Bash.init/1` returns `:bash_dep_unavailable` before this
    adapter is used.
    """
  end
end
