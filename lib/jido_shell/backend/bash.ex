if Code.ensure_loaded?(Bash.Session) do
  defmodule Jido.Shell.Backend.Bash do
    ...
    defp safe_signal_execution(pid) when is_pid(pid) do
      if Process.alive?(pid), do: signal_session(pid)
      :ok
    rescue
      _ -> :ok
    catch
    ...
    defp signal_session(pid) do
      cond do
        function_exported?(Bash.Session, :signal, 3) ->
          apply(Bash.Session, :signal, [pid, :sigint, [grace: @cancel_grace_ms]])

        function_exported?(Bash.Session, :signal_job, 3) ->
          apply(Bash.Session, :signal_job, [pid, 1, :sigint])
  ...
  defmodule Jido.Shell.Backend.Bash do
    @moduledoc """
    Placeholder Bash backend compiled when the optional `:bash` dependency is not
    available.

    Add the `:bash` package to the consuming application to enable this backend.
    """
    ...
    @impl true
    def init(_config), do: {:error, Error.command(:start_failed, %{reason: :bash_dep_unavailable})}

    @impl true
    def execute(_state, command, args, _exec_opts) do
      line = Enum.join([command | args], " ")
      {:error, Error.command(:start_failed, %{reason: :bash_dep_unavailable, line: line})}
    end

    @impl true
    def cancel(_state, _command_ref), do: {:error, :bash_dep_unavailable}

    @impl true
    def terminate(_state), do: :ok

    @impl true
    def cwd(state), do: {:ok, Map.get(state, :cwd, "/"), state}

    @impl true
    def cd(state, path) when is_binary(path), do: {:ok, Map.put(state, :cwd, path)}
